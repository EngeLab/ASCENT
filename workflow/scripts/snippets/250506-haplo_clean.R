# 250505: clone and segment-level major/minor haplotype
library(tidyverse)
library(parallel)
...
# Setup
segs_slot <- "refined"
clone_slot <- "clone_final"
patient_id <- "ALL40"
run <- "results/ALL-latest"
# cn_obj <- file.path(run, patient_id, paste0(patient_id,"-clones.Rds"))
# baf_file <- file.path(run, patient_id, paste0(patient_id,"-baf.txt.gz"))

# Test
cn_obj <- "results/DNA_37_SK_250502_Downsample/ALL40/clones/ALL40-final_clone_object-g10-10000.Rds"
baf_file <- "results/DNA_37_SK_250502_Downsample/ALL40/ALL40-baf.txt.gz"

# Load cn data
d <- read_rds(cn_obj)
plot_clone_heatmap(d,show_chr=T,only_divergent = T, clone_types = T)
clones <- d$cells %>% 
  filter(!is.na(!!sym(clone_slot))) %>% 
  select(cell=dna_library_id, clone=!!sym(clone_slot), timepoint)
nrow(d$bins$good)
table(d$cells$clone_final, useNA="always")

# Load SNP data
snp_cols <- c("chr", "pos", "cell", "hapA_count", "hapB_count", "ref", "alt", "vcf_ref", "vcf_alt", "ref_count", "alt_count", "ref_quality", "alt_quality", "original_genotype")
snps.all <- read_tsv(baf_file, col_names=snp_cols) %>% 
  left_join(clones, by="cell") %>% 
  mutate(chr=factor(chr, levels=paste0("chr",1:22)))
snps.all %>% distinct(chr, pos) %>% nrow
snps.all %>% group_by(cell) %>% summarize(n=n()) %>% pull(n) %>% hist(., main="snps/cell")
...
### BUG? 70 SEGS to start. 64 after removing XY. 61 after blocks.segs
# Missing: segments with no SNPs are left out
# Missing clone id in full join at the end. Do full join per clone? 
...
# Filter/zoom on section
# f <- 0:700
# seg_breaks <- seg_breaks %>% filter(start_block %in% f, end_block %in% f)
# seg_cn <- seg_cn %>% filter(start_block %in% f, end_block %in% f)
...
pdf("/wrk/data/solrun/AnalysisPipelinePaper/ALL40/Figure4/cmBAF.pdf", width=15, height=3)
s3
dev.off()
s2 + s3

### Make allele-specific copy numbers
# 250507: Added probability based genotype calls
source("workflow/scripts/snippets/250507-model_gt_wplots.R")
# TODO: Filters need some work
error_rate <- 0.02
error_multiplier <- 5
min_gt_prob <- 0.97 # or something (max 1)
# min_snp_segment <- 10 # already set earlier
# min_blocks_segment
...
#chr3 in 1_1 (and 5_1 but thats whatever)
#chr12 it switches which allele was gained in the second segment from 5_1 and 4_1 but only 5_1 is kept? 
#even though it's The smaller clone - it's because of the probability i guess? 
#chr14 - that fuckin segment it's cnLOH in 2_1 - we should just remove that segment 
#chr15 filtered out in the clones 4_1 and 5_1. - whatever
#chr16 small segment filtered out in 5_1 and 1_1 
#chr 20 + 22 - too small anyway 
#Is it here we look? baf_segs.l
baf_segs.l[[5]]%>%filter(chr=="chr12")%>%select(chr, arm, cn, n_blocks_all, gt_prob) 
baf_segs.l[[1]]%>%filter(chr=="chr3")%>%select(chr, arm, cn, n_blocks_all, gt_prob) 
baf_segs.l[[1]]%>%filter(chr=="chr3")  #Doesn't pass snp filter 
baf_segs.l[[3]]%>%filter(chr=="chr22") %>%select(chr, arm, cn, n_blocks_all, gt_prob)  #Doesn't pass snp filter 
baf_segs.l[[4]]%>%filter(chr=="chr14") %>%select(chr, arm, cn, n_blocks_all, gt_prob)  #Is this because of the gt_prob? Like why isn't this one here.. 
baf_segs.l[[2]]%>%filter(chr=="chr2") %>%select(chr, arm, cn, n_blocks_all, gt_prob)  #Is this because of the gt_prob? Like why isn't this one here.. 


### Outputs for medicc etc
medicc <- bafs %>% 
...
# Print medic input
# Optional exclude xy
# medicc <- medicc %>% filter(!chr %in% c("chrX","chrY"))
...
# o <- sort_from_diploid_root(cn_clones, na.rm = T)
...
#Yeah so I'm thinking now we make a new cn slot and use a diff color scale because 
baf_segs.l[[1]]$cn_a
baf_segs.l[[1]]$cn_b
...
d$clones$ab_copy <- lapply(baf_segs.l[d$clones$clone_id], function(x) x$cn_ab[rep(1:nrow(x), x$n.probes)])

plot_clone_heatmap(d, cn_slot="ab_copy", show_chr=T)
#Then we should make a new column that fills it with the cn original 
baf_segs.l[[1]]$cn_a_orig
baf_segs.l[[1]]$cn_b_orig

baf_segs.l <- map(baf_segs.l, function(df) {
  
  # Step 1: Create cn_ab_noNA as numeric (sum of cn_a_orig and cn_b_orig)
  df$cn_ab_noNA <- map2_dbl(df$cn_a_orig, df$cn_b_orig, function(a, b) {
    if (is.na(a) || is.na(b)) {
      NA_real_  # If either value is NA, return NA_real_ (numeric NA)
    } else {
      a + b  # Return the sum as a numeric value
    }
  })
  
  # Step 2: Create final_cn_ab based on ab_copy and cn_ab_noNA
  df$final_ab <- ifelse(
    is.na(df$cn_ab),  # If ab_copy is NA, take from cn_ab_noNA
    df$cn_ab_noNA,
    df$cn_ab  # Otherwise, take the value from ab_copy
  )
...
baf_segs.l[[1]]$final_ab

baf_segs.l[[1]]$cn_ab_noNA

#Ok now need to get rid of the ones that are diploid... 
#Problem segments are: 
#Clone 5_1 first segment on 14
#Clone 2_1 last segment on 14 

baf_segs.l[[5]]%>%filter(chr=="chr14")%>%view

write_rds(d, file=cn_obj)

pdf("/wrk/data/solrun/AnalysisPipelinePaper/ALL40/Figure4/clone_heatmap_notcnacnb.pdf", width=15, height=7)
plot_clone_heatmap(d, cn_slot="final_ab", show_chr=T, order=c("2_1", "5_1", "4_1", "1_1", "3_1"))
dev.off()


bafs<-do.call(rbind,baf_segs.l)
befs<-bafs%>%select(chr, clone, start.pos, end.pos, cn_a, cn_b, final_ab)
...
write_tsv(medicc, file="/wrk/data/solrun/AnalysisPipelinePaper/ALL40/Figure4/250508_medicc.tsv")
# Run medicc2 (in bash)
# conda activate medicc
# medicc2 -n 1_1 results/ALL-latest/ALL3/ALL3-medicc.txt results/ALL-latest/ALL3/medicc
