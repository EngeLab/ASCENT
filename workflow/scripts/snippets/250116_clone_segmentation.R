...
source("clone_functions_241214.R")

# Parameters
run <- "results/DNA_150_SKE/"
patient_id <- "enge26"
read_length<-150
sc_gamma <- 5
diploid_clone<-"5_4"
...
scalefactors_table<-read.table(paste0(run, "clones/", patient_id, "-scalefactors-g", sc_gamma, "-20000.txt"), col.names = c("dna_library_id", "scale_factor")) #Always g1 because thats where scp where calculated from 
...
bins_all <- load_bins(bins_file = paste0("resources/fixed-",binsize,".bed"),
                      ...
                      cytoband_file = "/wrk/resources/dntr/resources/cytoBand.txt.gz") #Do we need a new cytoband file? na?? 
...
df <- left_join(a, scalefactors_table) %>%filter(ploidy < 5) %>% mutate(copy = factor(ploidy))
...
cell_data <- load_and_join(metadata_files, join_by="cell_id") %>% 
  ...
  left_join(clones, by="dna_library_id")%>%left_join(scalefactors_table)%>%left_join(scp_2)
...
d <- calc_cn_integers(d, diploid_clone=diploid_clone) 
