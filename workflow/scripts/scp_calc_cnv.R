...
clones <- (read_tsv(snakemake@input[["clones"]]))
...
clone_list <- split(clones$dna_library_id, clones$clone_final)
cn_clone_median <- sapply(clone_list, function(cells) round(matrixStats::rowMedians(mtx_s[,cells],na.rm=T)))

# Lookup for cell vs clone
col_to_group <- setNames(rep(names(clone_list), sapply(clone_list, length)), nm=unlist(clone_list))

# Replace cell-level cn vector with clone median
cn_clone.segs <- mtx_s
cn_clone.segs[,names(col_to_group)] <- cn_clone_median[,col_to_group]

# Keep original cell-level cn in certain groups
exclude_groups <- c("0")
exclude_cells <- unlist(clone_list[exclude_groups],use.names=F)
cn_clone.segs[,exclude_cells] <- mtx_s[,exclude_cells]
