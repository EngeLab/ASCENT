#Get haplotype info outside pipeline
library(tidyverse)
library(parallel)
...
#Melanoma with tri and tetraploid regions
patient_id <- "SKZ080"
#Src with unexpected behaviour during filtering
#patient_id<-"src21"
run <- "results/Navin_DUKE254P/"

#cn_obj<-"final_clone_object.Rds"
d <- read_rds("results/Navin_DUKE254P/SKZ080/clones/SKZ080-final_clone_object_manual.Rds")
#d <- read_rds("results/DNA_37_SK/src21/clones/src21-final_clone_object_manual.Rds")
plot_clone_detail(d, region="chr1", clone_ids=c("26_1"))
plot_clone_heatmap(d, show_chr=T)
#Thingis sometimes its clone clean not clone final right? Need to be aware

# clones <- d$cells %>% 
#   filter(!is.na(clone_final)) %>% 
#   select(cell=dna_library_id, clone=clone_final)

segs_slot <- names(d[["segments"]])[length(d[["segments"]])] 
clone_slot <- names(d[["cells"]])[max(grep("clone_", names(d[["cells"]])))]
clone_slot<-"clone_final"
clones <- d$cells %>% 
  select(cell=dna_library_id, clone=clone_slot)%>%filter(!is.na(clone))
...
# I think rule three doesn't work for highly amplified cases...? 
...
#Analyse here 
plot_clone_heatmap(d, show_chr=TRUE)
...
plot_clone_detail(d, region="chr11", cn.trunc = 8) + s1
