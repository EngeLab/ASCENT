...
theme_dntr <- function (font_size = 14, font_family = "Helvetica", line_size = 0.5, axis_labels=T, axis_ticks=F, legend="bottom") {
  ...
  th <- theme_grey(base_size = font_size, base_family = font_family) %+replace% 
    theme(panel.background = element_blank(), panel.border = element_rect(fill = NA, colour = "black", size=1), 
          ...
          axis.ticks = element_line(size=ifelse(axis_ticks,0.5,0)),
...
call_segments <- function(data, 
  ...
  if(is.null(region)){
    ...
    if(is.null(region)){
      ...
      mpcf.input1 <- data.frame(
      chr = bins$chr_int[paste0(bins$chr_int, bins$arm)%in%region],
      pos = bins$start[paste0(bins$chr_int, bins$arm)%in%region],
      norm.mtx[paste0(bins$chr_int, bins$arm)%in%region,],
      check.names = F
    )
    
    mpcf.input2 <- data.frame(
      chr = bins$chr_int[!paste0(bins$chr_int, bins$arm)%in%region],
      pos = bins$start[!paste0(bins$chr_int, bins$arm)%in%region],
      norm.mtx[!paste0(bins$chr_int, bins$arm)%in%region,],
      check.names = F
    ...
    if(!is.null(norm_ratio)) {
      normY.mtx <- do.call(cbind, data[["clones"]][[norm_ratio]])
      colnames(normY.mtx) <- data[["clones"]][["clone_id"]]
      mpcf.inputY1 <- data.frame(
        chr = bins$chr_int[paste0(bins$chr_int, bins$arm)%in%region],
        pos = bins$start[paste0(bins$chr_int, bins$arm)%in%region],
        normY.mtx[paste0(bins$chr_int, bins$arm)%in%region,],
        check.names=F
      )
      normY.mtx <- do.call(cbind, data[["clones"]][[norm_ratio]])
      colnames(normY.mtx) <- data[["clones"]][["clone_id"]]
      mpcf.inputY2 <- data.frame(
        chr = bins$chr_int[!paste0(bins$chr_int, bins$arm)%in%region],
        pos = bins$start[!paste0(bins$chr_int, bins$arm)%in%region],
        normY.mtx[!paste0(bins$chr_int, bins$arm)%in%region,],
        check.names=F
    ...
    res1 <- copynumber::multipcf(
      data = mpcf.input1,
      Y = mpcf.inputY1,
      pos.unit = "bp",
      arms = bins$arm[paste0(bins$chr_int, bins$arm)%in%region],
      gamma = gamma_region,
      normalize = FALSE,
      fast = FALSE,
      verbose = TRUE,
    ...
    segs_all<-rbind(segs1, segs2)
    segs_all$original_row <- 1:nrow(segs_all)
    segs_sorted <- segs_all[order(segs_all$chr, segs_all$arm, segs_all$start.pos), ]
    segs_sorted$start<-c(1, cumsum(segs_sorted$n.probes)[-length(segs_sorted$n.probes)] + 1)
    segs_sorted$end<-cumsum(segs_sorted$n.probes)
    segs_sorted$seg_idx <- seq_along(segs_sorted$seg_idx)
    rownames(segs_sorted)<-seq_along(rownames(segs_sorted))
    data[["segments"]][[segs_slot]] <- segs_sorted[,!colnames(segs_sorted)%in%"original_row"]
    res_all_segments<-rbind(res1$segments[,-c(1:5)], res2$segments[,-c(1:5)])
    res_all_estimates<-rbind(res1$estimates[,-c(1:2)], res2$estimates[,-c(1:2)])
    res_all_seg<-res_all_segments[segs_sorted$original_row,]
    res_all_est<-res_all_estimates[segs_sorted$original_row,] 
    
    data[["clones"]][["segment_means"]] <- as.list(as.data.frame(res_all_seg))
    data[["clones"]][["segment_bins"]] <- as.list(as.data.frame(res_all_est))
    return(data)
  }
}
...
mask_high_residuals <- function(data, segs_slot = NULL, new_slot = "masked", max_residual=0.3, clone_filter_fraction=0.3, update_clones=T, diploid_clone=diploid_clone){
  ...
  #so now its not the correct residuals - these are residuals for everything - so need the indexes for the right ones??
  ...
  cat(sprintf("Found %d segments with high residuals. Masked as NA", sum(high_residual_vals,na.rm = T)))
  ...
  if(update_clones)  data <- update_clone_metrics(data, norm_slot = "gcmap_normal", segs_slot=new_slot, calc_cn=T, diploid_clone=diploid_clone)
  
  return(data)
}

update_clone_metrics <- function(data, norm_slot="gcmap_normal", segs_slot=NULL, calc_cn=TRUE, subset=NULL, diploid_clone=diploid_clone){
  ...
  # First run or re-calculating cn
  ...
  if(calc_cn) data <- calc_cn_integers(data, subset=subset, diploid_clone=diploid_clone)
  
  return(data)
}

calc_cn_integers <- function(data, subset=NULL, scale_range=c(1.5, 4.5), diploid_clone=diploid_clone, clone_slot=NULL){
  #Here: Need to get summary_stats from the clones used 
  if(is.null(clone_slot)) clone_slot <- names(data[["cells"]])[max(grep("clone_", names(data[["cells"]])))]
  clone_data <- tibble(clone = data[["clones"]]$clone_id, dna_library_id = data[["clones"]]$cells) %>% unnest(dna_library_id) %>% left_join(scp_2)%>%left_join(scalefactors_table)
  summary_stats2 <- clone_data %>%
    group_by(clone) %>%
    summarise(
      mean_coverage_at_2x = median(depth2, na.rm=TRUE),
      mean_scale_factor = mean(scale_factor, na.rm=TRUE),
      sd_cov = sd(depth2, na.rm=TRUE)
    ) %>%
    ungroup() %>%
    mutate(
      diploid_coverage = mean_coverage_at_2x[clone == diploid_clone],
      sd_cov_dip=sd_cov[clone==diploid_clone],
      scalefactor = case_when(
        mean_coverage_at_2x >= (diploid_coverage - 2.5 * sd_cov_dip) & mean_coverage_at_2x <= (diploid_coverage + 2.5 * sd_cov_dip) ~ "good",  
        mean_coverage_at_2x < (diploid_coverage - 2.5 * sd_cov_dip) ~ "low",    
        mean_coverage_at_2x > (diploid_coverage + 2.5 * sd_cov_dip) ~ "high",   
      ),
      min_scale_factor = case_when(
          scalefactor == "good" ~ (min_scale_factor=pmax(mean_scale_factor - 0.5, 1)),
          scalefactor == "low" ~ (min_scale_factor=pmax(mean_scale_factor - 1, 1)), 
          scalefactor == "high" ~(min_scale_facor=(mean_scale_factor + 0.5))
      ),
        max_scale_factor = case_when(
          scalefactor == "good" ~(max_scale_factor = mean_scale_factor + 0.5),
          scalefactor == "low" ~ (max_scale_factor = mean_scale_factor),
          scalefactor == "high" ~ (max_scale_factor = mean_scale_factor + 2.5)
      )
      )
  ...
  #I need the index for the cells I will actually use - -

  for(i in 1:length(data[["clones"]]$clone_id)){
    clone_name<-data[["clones"]][i,]$clone_id
    scalefactors <- summary_stats2 %>%
      filter(clone == clone_name) %>%
      select(min_scale_factor, max_scale_factor)
    if (nrow(scalefactors) == 0 || is.na(scalefactors$min_scale_factor)) {
      s <- seq(1.5, 4.5, length.out = 100)
    } else {
      s <- seq(scalefactors$min_scale_factor[1], scalefactors$max_scale_factor[1], length.out = 100)
    }
    print(scalefactors)
    cat(paste0("\nCalculating scale factors and copy number integers\n", scalefactors$min_scale_factor, scalefactors$max_scale_factor, " in clone ", clone_name, "\n"))
    data[["clones"]][i,] <- data[["clones"]][i,] %>% 
      mutate(
        scale_tests = map(segment_bins, ~sapply(s, function(sf) manhattan.dist(sf, .x, na.rm=TRUE))),
...
split_mixed_clones <- function(data, segs_slot=NULL, cells_slot="gcmap_normal", 
                               residual_threshold = 0.3, min_cells = 10,
                               improvement_threshold = 0.8, total_improvement_threshold=1.5,
                               plot=TRUE, verbose=TRUE, update_clones=TRUE, diploid_clone=diploid_clone) {
  ...
  for(i in seq_along(data$clones$clone_id)) {
    ...
    for(g in seq_along(ordered_groups)) {
      ...
      for(node_idx in seq_along(current_nodes)) {
        ...
        #Shouldn't this be region_improvement>
  ...
  if(length(split_results)>0){
    ...
    if(update_clones){
      ...
      data <- update_clone_metrics(data, subset=all_new_clones, calc_cn=TRUE, diploid_clone=diploid_clone)
    }
...
merge_duplicate_clones <- function(data, segs_slot=NULL, clone_slot=NULL, diploid_clone=diploid_clone){
  if(is.null(segs_slot)) segs_slot <- names(data[["segments"]])[length(data[["segments"]])]
  if(is.null(clone_slot)) clone_slot <- names(data[["cells"]])[max(grep("clone_", names(data[["cells"]])))]
  ...
  if(nrow(merge_table)>0){
    ...
    data <- update_clone_metrics(data, subset=new_clones.names, calc_cn=TRUE, diploid_clone=diploid_clone)
...
merge_almost_duplicate_clones <- function(data, segs_slot=NULL, clone_slot=NULL, diploid_clone=diploid_clone){
  if(is.null(segs_slot)) segs_slot <- names(data[["segments"]])[length(data[["segments"]])]
  if(is.null(clone_slot)) clone_slot <- names(data[["cells"]])[max(grep("clone_", names(data[["cells"]])))]
  cat("\n\n> Merging duplicate clones",
      "\nSegments slot:",segs_slot,
  ...
  result <- similarity_check(m, threshold = 0.05)
  
  check_res_values <- function(result, res, threshold = 0.3) {
  ...
  output <- check_res_values(result, r, threshold = 0.3)
...
remove_bad_clones<-function(data, clone_slot=NULL, frequency=0.1, diploid_clone=diploid_clone){
  if(is.null(clone_slot)) clone_slot <- names(data[["cells"]])[max(grep("clone_", names(data[["cells"]])))]
...
remove_small_clones<-function(data, clone_slot=NULL,  diploid_clone=diploid_clone, min_size_clone=min_size_clone){
  if(is.null(clone_slot)) clone_slot <- names(data[["cells"]])[max(grep("clone_", names(data[["cells"]])))]
  ...
  new_clones.name<-data[["clones"]]$clone_id[!data[["clones"]]$clone_id%in%bad_clones]
  data[["cells"]][["clone_clean2"]]<-mgsub(data[["cells"]][[clone_slot]], (clone_to_zero), c(rep("0", length(clone_to_zero))))
...
refine_segments_from_cn <- function(data, new_slot="refined", update_clones=TRUE, diploid_clone=diploid_clone) {
  bins <- data[["bins"]][["good"]]
  ...
  data[["segments"]][[new_slot]] <- data.frame(
    chr = bins$chr_int[refined_segs$start],
...
recall_cells <- function(data, mad_cutoff=5, segs_slot=NULL, clone_slot=NULL, plot=TRUE){
  if(is.null(segs_slot)) segs_slot <- names(data[["segments"]])[length(data[["segments"]])]
  if(is.null(clone_slot)) clone_slot <- names(data[["cells"]])[max(grep("clone_", names(data[["cells"]])))]
  
  cat(sprintf("\n\n> Clone recall. Using segs from %s and clones from %s\n", segs_slot, clone_slot))
  ...
  data$cells <- data$cells %>% 
    rowwise() %>% 
    mutate(
      raw_counts_segs = if_else(
        !is.na(!!sym(clone_slot)),
        list({
          r <- raw_counts[data$bins$all$good]
  ...
  # Scale per row to 0-1 (reverse scale so smallest distance = 1)
  # recall_cor_scaled <- t(apply(recall_cor, 1, function(x) {
  #   1 - (x - min(x))/(max(x) - min(x))
  # }))
  # Convert distances to probabilities per row (smallest distance = highest probability)
  # recall_cor_scaled <- t(apply(recall_cor, 1, function(x) {
  #   # Convert distances to probabilities (inverse and normalize)
  #   x_inv <- 1/x  # Inverse so smaller distances become larger values
  #   x_inv/sum(x_inv)  # Normalize to sum to 1
  # }))
  recall_cor_scaled <- rank_to_prob_exp(recall_cor)
  
  if(plot){
    clone_colors_col <- get_clones_col(c("0",colnames(recall_cor)))
    cell_clones <- data[["cells"]][[clone_slot]][match(rownames(recall_cor),data$cells$dna_library_id)]
    clone_colors_row <- clone_colors_col[cell_clones]
    ha_col <- HeatmapAnnotation(clone = colnames(recall_cor), col = list(clone = clone_colors_col))
    ha_row <- rowAnnotation(original_clone = cell_clones, col = list(original_clone = clone_colors_row))
    ht <- Heatmap(recall_cor_scaled,
            cluster_columns = T,
            cluster_rows = T,
  ...
  l.recall_clone <- apply(recall_cor, 1, function(x) {
    ifelse(min(x) < recall_cutoff, colnames(recall_cor)[which.min(x)], "0")
  })
  
  #Here only match the ones that were zero to something else 
  ...
  data$cells$clone_recall[match_idx] <- unlist(l.recall_clone)
  
  #So if we compare the two of these that should be possible 

  data$cells<-data$cells %>%
    mutate(clone_zero_recall = if_else(.data[[clone_slot]] == "0" & !is.na(clone_recall), clone_recall, .data[[clone_slot]]))
  
  
  print(table("old"=data$cells[[clone_slot]], "recall"=data$cells$clone_zero_recall))
...
recall_replicating_cells <- function(data, phase_slot="cell_phase", phase_calls=c("S","G2M"), min_counts=50000L, segs_slot=NULL, clone_slot=NULL, mad_cutoff=10, plot=TRUE){
  if(is.null(segs_slot)) segs_slot <- names(data[["segments"]])[length(data[["segments"]])]
  if(is.null(clone_slot)) clone_slot <- names(data[["cells"]])[max(grep("clone_", names(data[["cells"]])))]
  cat(sprintf("\n\n> Match replicating (S/G2M phase) cells with closest clone profile. Using segs from %s, clones from %s and phase annotation from %s\n", segs_slot, clone_slot, phase_slot))
  stopifnot("Cell phase slot not found"=phase_slot %in% colnames(data$cells))
  
  cell_idx <- data[["cells"]][[phase_slot]] %in% phase_calls & sapply(data$cells$raw_counts, function(x) sum(x) > min_counts)
  if(sum(cell_idx)==0) stop(sprintf("No cells in phase_slot: %s matching phase calls: %s", phase_slot, paste0(phase_calls,collapse=", ")))
  cat(sprintf("> Processing %s replicating cells with bincounts > %d\n", sum(cell_idx), min_counts))
  
  segs <- data[["segments"]][[segs_slot]]
  seg_idx <- Map(`:`, segs$start, segs$end)
  
  data$cells[cell_idx,] <- data$cells[cell_idx,] %>% 
    mutate(
  ...
  cell_counts <- do.call(cbind, data$cells$raw_counts_segs[cell_idx])
  colnames(cell_counts) <- data$cells$dna_library_id[cell_idx]
  cell_counts.n <- t(t(cell_counts) / colSums(cell_counts)) # OBS! Have to transpose since R does -first division
  ...
  if(plot){
    clone_colors_col <- get_clones_col(c("0",colnames(recall_cor)))
    cell_phase <- data[["cells"]][[phase_slot]][match(rownames(recall_cor),data$cells$dna_library_id)]
    ha_col <- HeatmapAnnotation(clone = colnames(recall_cor), col = list(clone = clone_colors_col))
    ha_row <- rowAnnotation(cell_phase = cell_phase, col = list(cell_phase = phase_cols))
    ht <- Heatmap(recall_cor_scaled,
                  cluster_columns = T,
                  cluster_rows = T,
                  col = colorRamp2(c(0, 1), c("white", "black")),
  ...
  l.recall_prob <- asplit(recall_cor_scaled, 1)
  recall_cutoff <- median(l.recall_dist_min) + (mad(l.recall_dist_min) * mad_cutoff)
  phases <- data[["cells"]][[phase_slot]][cell_idx]
  l.recall_clone <- sapply(1:nrow(recall_cor), function(i) {
    x <- recall_cor[i,]
    r <- ifelse(min(x) < recall_cutoff, paste0(colnames(recall_cor)[which.min(x)],"_",phases[i]), "0")
    return(r)
  })
  
  match_idx <- match(rownames(recall_cor), data$cells$dna_library_id)
  if(is.null(data$cells$recall_dist)) data$cells$recall_dist <- list(NULL)
  if(is.null(data$cells$recall_prob)) data$cells$recall_prob <- list(NULL)
  if(is.null(data$cells$recall_prob)) data$cells$clone_recall <- NA
  
  data$cells$recall_dist[match_idx] <- l.recall_dist
  data$cells$recall_prob[match_idx] <- l.recall_prob
  data$cells$clone_recall[match_idx] <- unlist(l.recall_clone)
  
  print(table("phase"=data[["cells"]][[phase_slot]][cell_idx], "clone"=data[["cells"]][[clone_slot]][cell_idx]))
  
  return(data)
}

calc_cell_cn <- function(data, cell_idx=NULL, force_cn=FALSE, cells_slot="gcmap_normal", segs_slot=NULL, clone_slot=NULL, diploid_clone=diploid_clone){
  ...
  # Cells to process, do not recalculate unless forced
  if(is.null(cell_idx)){
    clone_idx <- !is.na(data[["cells"]][["clone_initial"]])
    norm_idx <- clone_idx & sapply(data[["cells"]][[cells_slot]], is.null)
    cn_idx <- clone_idx & sapply(data[["cells"]][["cn"]], is.null) 
  } else {
  ...
  data[["cells"]][["segment_means"]][cn_idx] <- lapply(data[["cells"]][[cells_slot]][cn_idx], function(x){
    ...
    #r[segs$filtered] <- NA #Lets keep the filtered segments for this - this time at least 
    r
  })
  data[["cells"]][["segment_bins"]][cn_idx] <- lapply(data[["cells"]][["segment_means"]][cn_idx], function(x) rep(x, segs$n.probes))
  #Here need to do some magic... 
  
  cell_clones<-data$cells[cn_idx,c("dna_library_id", clone_slot)]
  for(i in 1:length(which(cn_idx))){
    print(i)
    scalefactors <- summary_stats2 %>%
      filter(clone == cell_clones[[clone_slot]][i]) %>%
      select(min_scale_factor, max_scale_factor)
    if (nrow(scalefactors) == 0 || is.na(scalefactors$min_scale_factor)) {
      s <- seq(1.5, 4.5, length.out = 100)
    } else {
    ...
    data[["cells"]][["scale_tests"]][which(cn_idx)[i]] <-map(data[["cells"]][["segment_bins"]][which(cn_idx)[i]], ~sapply(s, function(sf) manhattan.dist(sf, .x, na.rm = T)))
    data[["cells"]][["scale_factor"]][which(cn_idx)[i]]  <- lapply(data[["cells"]][["scale_tests"]][which(cn_idx)[i]], function(x) s[which.min(x)])
    }
  ...
  # data[["cells"]][["residuals"]][cn_idx] <- lapply(which(cn_idx), function(i){
  #   abs(data[["cells"]][["segment_means"]][[i]] * data[["cells"]][["scale_factor"]][[i]] - data[["cells"]][["cn"]][i])
  # })
  return(data)
}
...
remove_bad_cells<-function(data, clone_slot=NULL, diploid_clone=diploid_clone,  update_clones=T, max_diff_bins=max_diff_bins, segs_slot=NULL){
  if(is.null(clone_slot)) clone_slot <- names(data[["cells"]])[max(grep("clone_", names(data[["cells"]])))]
  if(is.null(segs_slot)) segs_slot <- names(data[["segments"]])[length(data[["segments"]])]
  ...
  comparison_results <- lapply(names(clone_list), function(clone) {
    lapply(clone_list[[clone]], function(cell){
      cell_cn<-unlist(data[["cells"]][["cn"]][data[["cells"]]$dna_library_id==cell]) #CN for that cell
      clone_cn<-unlist(data[["clones"]]["cn"]$cn[data[["clones"]]$clone_id==clone])
      segm_use<-data[["segments"]][[segs_slot]][,c("start", "end")][data[["segments"]][[segs_slot]]$n.probes>500,]
      #Subset here for only large segments and then we should be able to  
      segm_use
      subset_cell_cn <- unlist(lapply(1:nrow(segm_use), function(i) {
        cell_cn[segm_use$start[i]:segm_use$end[i]]
      }))
      subset_clone_cn <- unlist(lapply(1:nrow(segm_use), function(i) {
        clone_cn[segm_use$start[i]:segm_use$end[i]]
      }))
      if(sum(subset_cell_cn!=subset_clone_cn, na.rm=TRUE)>max_diff_bins){
        #Move the cell to clone zero - -  
  ...
  print(table("old"=data$cells[[clone_slot]], data$cells$clone_final))
  data <- update_clone_metrics(data, calc_cn=TRUE, diploid_clone=diploid_clone)
  return(data)
}
...
plot_cell_heatmap <- function(data, clone_slot=NULL, smooth_bins=50, annotate=NULL, annotate_colors=NULL, segs_slot=NULL){
  if(is.null(clone_slot)) clone_slot <- names(data[["cells"]])[max(grep("clone_", names(data[["cells"]])))]
  if(is.null(segs_slot)) segs_slot <- names(data[["segments"]])[length(data[["segments"]])] 
  bins <- data$bins$good
  clone_idx <- !is.na(data[["cells"]][["clone_initial"]])
  ...
  clone_list <- split(data[["cells"]][["dna_library_id"]][clone_idx], data[["cells"]][[clone_slot]][clone_idx])
  clone_list<-clone_list[!names(clone_list)%in%"0"]
  
  clone_medians <- lapply(clone_list, function(i) Rfast::rowMedians(recall_cn[,i]))
  
  bins$chr_short <- sub("chr","",bins$chr)
  smooth <- smooth_bins_subset(recall_cn, bins=bins, w=smooth_bins) # TODO: Add S/G2M but drop bad QC
  ht_mtx <- smooth$mat
  filtered<-rep(data[["segments"]][[segs_slot]]$filtered, data[["segments"]][[segs_slot]]$n.probes)
  smoothed_filtered <- smooth_vector(filtered, 50)
  
  #SO here need to figure out the segments from here..? 
  ...
  meta[,clone_vars]<-meta[,clone_vars] %>%
    mutate(across(everything(), ~replace_na(.x, "0")))
  clone_colors <- get_clones_col(unique(unlist(meta[, clone_vars])))
  ...
  annot_data <- sapply(clone_vars, function(col) meta[[col]], simplify=FALSE)
  annot_colors <- rep(list(clone_colors), length(clone_vars)) #Why is there NA here now..... 
  ...
  ha <- rowAnnotation(
    df = annot_data,
    col = annot_colors,
    #recall = anno_barplot(recall_prob, gp = gpar(col=recall_colors, fill = recall_colors)),
    show_legend=T,
  ...
  column_anno = HeatmapAnnotation(
    seg_type = smoothed_filtered,
    col = list(seg_type = c("TRUE" = "red", "FALSE"=NA),
  ...
  ht = Heatmap(t(ht_mtx), name="cn", col=col_fun,
               ...
               cluster_columns = F,
               #row_split = meta$clone_recall, # Final clone set
               row_split = meta[clone_slot], # Final clone set
               #row_split = meta$clone_merged,
               # row_order = cells.final_order, # Named vector matching rownames in mtx
...
summarize_stats <- function(df, diploid_clone) {
  df %>%
    filter(copy == 2) %>%
    group_by(clone) %>%
    summarise(
    ...
    mutate(
      ...
      sd_cov_dip = sd_cov[clone == diploid_clone],
      scalefactor = case_when(
        mean_coverage_at_2x >= (diploid_coverage - 2.5 * sd_cov_dip) & 
          mean_coverage_at_2x <= (diploid_coverage + 2.5 * sd_cov_dip) ~ "good",
        mean_coverage_at_2x < (diploid_coverage - 2.5 * sd_cov_dip) ~ "low",
        mean_coverage_at_2x > (diploid_coverage + 2.5 * sd_cov_dip) ~ "high"
      ),
      min_scale_factor = case_when(
        scalefactor == "good" ~ pmax(mean_scale_factor - 0.5, 1),
        scalefactor == "low" ~ pmax(mean_scale_factor - 1, 1),
        scalefactor == "high" ~ mean_scale_factor + 0.5
      ),
      max_scale_factor = case_when(
        scalefactor == "good" ~ mean_scale_factor + 0.5,
        scalefactor == "low" ~ mean_scale_factor,
        scalefactor == "high" ~ mean_scale_factor + 2.5
      )
    )
}
...
parse_region <- function(region) {
  ...
  if (grepl("^chr\\d{1,2}[pq]$", region)) {
    # Chromosome arm format: chr12p
    chr <- sub("([pq])$", "", region)
    arm <- substr(region, nchar(region), nchar(region))
    return(list(type="arm", chr=chr, arm=arm))
    
  } else if (grepl("^chr\\d{1,2}:\\d+\\-\\d+[kKmMbB]*$", region)) {
    # Position range format: chr12:0-12Mb
    chr <- sub(":.*$", "", region)
    range_str <- sub("^.*:", "", region)
    positions <- strsplit(range_str, "-")[[1]]
    
    # Convert to numeric, handling k/m/b suffixes
  ...
  } else if (grepl("^chr\\d{1,2}$", region)) {
    # Whole chromosome format: chr12
    return(list(type="chromosome", chr=region))
    
  } else {
    stop("Invalid region format. Use 'chr12', 'chr12p', or 'chr12:0-12Mb' format")
  }
}
...
get_segment_highlights <- function(segments, end_cum) {
  seg.grid.labs <- segments %>% 
    mutate(
      # Switch chr naming if necessary
      chr = ifelse(grepl("^chr",chr), 
                   chr, 
                   ifelse(chr == "23", "chrX",
                          ifelse(chr == "24", "chrY",
                                 paste0("chr",chr)))
...
plot_clone_detail <- function(data, show_raw=TRUE, show_norm=TRUE, show_segs=TRUE,
                              cn.trunc=7, labels=NULL, region=NULL, clone_ids=NULL,
                              segs_slot=NULL) {
  if(is.null(segs_slot)) segs_slot <- names(data[["segments"]])[length(data[["segments"]])] 
  if(!segs_slot %in% names(data[["segments"]])) stop("Specified segment slot not found")
  ...
  if(length(unique(df$chr)) > 1 && length(unique(df$chr)) < 24) {
    ...
    p <- p +
      ...
      facet_wrap(
        vars(clone),
        ncol=1,
        labeller=labeller(clone=labels),
        scales="free_x"
      )
  }
...
### OLD STUFF
### Functions to create
# Get clonal and subclonal changes (part or all clones, treat clone closest to cn=2 as normal)
# Get cancer gene census overlap
# Grid with annotations, highlight cancer gene census genes or any gene if <5. if >5 just *
# With complexHeatmap

# filter_bad_clones <- function()
# recall_cells <- function()
# 
# cor_clust_plot <- function(x, label="bins", order=NULL, na.rm=T){
#   if(ncol(x)<=1) {
#     p <- ggplot() + annotate("text", x=0.5, y=0.5, label=paste0("Too few clones (n=",ncol(x),")")) + theme_void()
#     return(p)
#   }
#   diff <- outer(1:ncol(x), 1:ncol(x), 
#                 Vectorize(function(i, j) sum(x[,i]!=x[,j], na.rm = na.rm)))
...
#   } else {
#     df <- df %>% mutate(clone=factor(clone, levels=hc$labels[hc$order]),
#                         comparison=factor(comparison, levels=hc$labels[hc$order]))
#   }
#   
