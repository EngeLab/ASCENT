...
calc_cn_integers_knownScaleFact <- function(data, subset=NULL, scale=scale_facotrs){
  run_idx = TRUE # Default run all clones
  if(!is.null(subset)) {
    run_idx <- data[["clones"]][["clone_id"]] %in% subset
    if(sum(run_idx)==0) stop("Subset does not match any clones")
  ...
  data[["clones"]]$median_scale_factor <- map_dbl(data[["clones"]]$cells, function(cell_list) {
    scale %>%
      filter(dna_library_id %in% cell_list) %>%
      summarize(median_scale_factor = median(scale_factor_adjusted, na.rm = TRUE)) %>%
      pull(median_scale_factor)
  })
  
  # Run update for selected clones
  data[["clones"]][run_idx,] <- data[["clones"]][run_idx,] %>% 
    mutate(
      s_values = map(median_scale_factor, ~ seq(.x - 0.5, .x + 0.5, by = 0.1)),
      
      # Apply computations with the new s_values
      scale_tests = map2(segment_bins, s_values, ~sapply(.y, function(sf) manhattan.dist(sf, .x, na.rm = TRUE))),
      scale_factor = map2(scale_tests, s_values, function(st, sv) sv[which.min(st)]),
      cn = map2(segment_bins, scale_factor, ~round(.x * .y)),  # Compute CN
      residuals = map2(segment_means, scale_factor, ~abs(.x * .y - round(.x * .y))) # Compute residuals
    )%>%select(!s_values)
...
plot_cell_heatmap <- function(data, filtered=TRUE, clone_slot=NULL, segs_slot=NULL, 
  ...
  meta <- meta %>%
    mutate(
      pass_dna_qc = !is.na(clone_initial) | cell_phase %in% c("S","G2M"),
      across(starts_with("clone_"), 
             ~case_when(
               !is.na(.) ~ .,
               pass_dna_qc == FALSE ~ "fail_qc",
               pass_dna_qc == TRUE ~ cell_phase
             )))
  ...
  ht <- draw(ht,
             column_title = paste0(patient_id, ", n=",ncol(ht_mtx),
                                   ", gamma=",data$parameters$clone_gamma,
                                   ", ", nrow(data[["segments"]][[segs_slot]]), 
                                   " segs, frags per bin: ", 
                                   round(mean(meta$bam_read_pairs)/nrow(data$bins$all),2),
                                   region_title),
             column_title_side = "top")
}
