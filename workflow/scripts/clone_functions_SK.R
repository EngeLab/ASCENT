...
# calc_cn_integers_SK <- function(data, subset=NULL, scale_range=c(1.5, 4.5), diploid_clone=diploid_clone, clone_slot=NULL){
...
calc_cn_integers <- function(data, subset=NULL, scale_range=c(1.5, 4.5)){
...
recall_cells <- function(data, mad_cutoff=5, segs_slot=NULL, clone_slot=NULL, plot=TRUE){
  ...
  # data$cells$clone_merged
  # data$cells$clone_recall
  # data$cells<-data$cells %>%
  #   mutate(clone_zero_recall = if_else(clone_merged == "0" & !is.na(clone_recall), clone_recall, clone_merged))
  
  
  print(table("old"=data$cells[[clone_slot]], "recall"=data$cells$clone_recall))
...
plot_cell_heatmap <- function(data, clone_slot=NULL, smooth_bins=50, annotate=NULL, annotate_colors=NULL){
  if(is.null(clone_slot)) clone_slot <- names(data[["cells"]])[max(grep("clone_", names(data[["cells"]])))]
  bins <- data$bins$good
  clone_idx <- !is.na(data[["cells"]][["clone_initial"]])
