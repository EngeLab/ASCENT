...
#    $stdist = 0; # Uhm I think.
	...
	#print STDERR "scRQ2: ", scalar @readQueue, "\n";
	$notEnd = <>;
	if($notEnd) {
	    chomp $notEnd;
		...
		#	    print @{$readQueue[0]};
#		print $#readQueue, "\n";
		$stdist = $readQueue[0][2] - $readQueue[$#readQueue][1];
		if($readQueue[0][0] ne $readQueue[$#readQueue][0]) {
		    $stdist = 0;
		}
		#print STDERR "stdist:, ", $stdist, "\n";
	    }
	}
    }
    #print STDERR "RQ3: ", scalar @readQueue, "\n";
    if(scalar @readQueue == 1) { # Only happens last iteration
	next;
    }
    my $md = $readQueue[1][1] - $readQueue[0][2];
#    print "Min dist ", $md, "\n";
#    if(scalar @readQueue == 1 || $stdist > $mindist) { # No dup candidates
	...
	#print STDERR "Maybe dup\n";
	print join("\t", @{$readQueue[0]}), "\n";
	my @newarr = ();
	for(my $i = 1; $i != @readQueue; ++$i) {
	    # Check if any of the reads are dups of the first one
	    my $dist1 = $readQueue[0][1] - $readQueue[$i][1];
	    my $dist2 = $readQueue[$i][2] - $readQueue[0][2];
...
#		print "No dup ", join ("\t", @{$readQueue[0]}), "\n-------", join ("\t", @{$newarr[$#newarr]}), "\n";
	...
	if(scalar @readQueue > 1) {
	    ...
	    #print STDERR "Overflowing stdist: ", $stdist, "\n";
	}
...
# Add check before final print
if (scalar @readQueue > 0) {
    print join("\t", @{$readQueue[0]}), "\n";
}